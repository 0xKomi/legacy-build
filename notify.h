...
public:
	...
	void think() {
		...
		for (size_t i{}; i < m_notify_text.size(); ++i) {
			...
			render::menu_shade.semi_filled_text( x, y, color, XOR( "\ [ andromeda ] \ " ), render::ALIGN_LEFT, left );
			render::menu_shade.semi_filled_text( x + render::menu_shade.size( XOR( "\ [ andromeda ] \ " ) ).m_width, y, log_color, notify->m_text, render::ALIGN_LEFT, left );
			y += size;
		}
	}
};
